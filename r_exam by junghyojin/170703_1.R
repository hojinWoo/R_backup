...
#산점도 x마다 y마다 상위 5프로 하위 5프로를 잘라내고(축 별로!), 나머지로 평균구함
#전체 데이터에서 위치상 잘라내는게아니라 각 데이터마다 잘라냄
#하나의 선으로 만드는게 아니라 분위수마다 여러개의 분포를 찾아낼 수 있음


## y축 관해서 정렬 시작 
n = round((length(iris$Sepal.Length)*0.1)/2,0)  # round(올림) -> 8이 나옴
n
sort(iris$Sepal.Length)

sort(iris$Sepal.Length)[n+1:length(iris$Sepal.Length)-n] # 오름차순으로 정렬

x1=sort(iris$Sepal.Length)[(n+1):(length(iris$Sepal.Length)-n)] # 앞에서 9번째부터 뒤에서 8번째까지 나타냄. 이게 백터라서 legnth
str(x1)  # sort를 이용했기 때문에 데이터 타입이 num으로 바뀜

m= round(length(iris$Petal.Length)*0.1/2,0)
m

sort(iris$Petal.Length)
y1=sort(iris$Petal.Length)[(m+1):(length(iris$Petal.Length)-m)]
